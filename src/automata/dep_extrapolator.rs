...
pub struct DepExtrapolator {
    stack: Vec<Literal>,
}

impl DepExtrapolator {
    ...
    pub fn extrapolate_expression(federation: Federation, expression: &Expression) -> Federation {
        DepExtrapolator::empty().expression(federation, expression)
    }

    pub fn extrapolate_statement(federation: Federation, statement: &Statement) -> Federation {
        DepExtrapolator::empty().statement(federation, statement)
    }
}
